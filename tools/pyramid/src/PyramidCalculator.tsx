...
function PyramidCalculator() {
    ...
    // Tính góc ở đỉnh và khoảng trống
    const apexAngle = 180 - 2 * angleInDegrees; // Góc ở đỉnh tam giác
    const halfApex = apexAngle / 2;
    const totalAngle = 4 * apexAngle; // Tổng góc chiếm bởi 4 tam giác
    const gapAngle = 360 - totalAngle; // Khoảng trống
    const gapAngleRad = gapAngle * Math.PI / 180;
    const gapLength = 2 * radius * Math.sin(gapAngleRad / 2); // Khoảng cách giữa 2 đỉnh ngoài cùng
        ...
        <div className="max-w-4xl mx-auto p-6 bg-gradient-to-br from-amber-50 to-orange-50 min-h-screen">
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                    <svg viewBox="0 0 500 500" className="w-full max-w-2xl mx-auto">
                        ...
                        {(() => {
                            ...
                            // Tính điểm giữa đường đỏ
                            const midX = (points[0].x + points[4].x) / 2;
                            const midY = (points[0].y + points[4].y) / 2;
                            ...
                            return (
                                <>
                                    ...
                                    {/* Đường thẳng màu đỏ nối 2 đỉnh ngoài cùng */}
                                    <line
                                        x1={points[0].x}
                                        y1={points[0].y}
                                        x2={points[4].x}
                                    ...
                                    {/* 2 điểm đầu mút của đường đỏ */}
                                    <circle cx={points[0].x} cy={points[0].y} r="6" fill="#ef4444" />
                                    <circle cx={points[4].x} cy={points[4].y} r="6" fill="#ef4444" />

                                    {/* Kích thước đường đỏ - đặt ngay trên đường đỏ */}
                                    <text
                                        x={midX}
                                        y={midY - 15}
                                        textAnchor="middle"
            ...
            <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-orange-50 p-4 rounded-lg border-2 border-red-300">
                        <h3 className="font-bold text-red-800 mb-2">🔴 Khoảng trống (đường thẳng đỏ):</h3>
                        <p className="text-2xl font-bold text-red-600">{gapLength.toFixed(2)} cm</p>
                        <p className="text-sm text-gray-600">Khoảng cách giữa 2 đỉnh đáy ngoài cùng</p>
                    </div>
                </div>
            </div>
